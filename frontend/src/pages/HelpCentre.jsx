...
const STEPS = [
  ...
  { n: '02', title: 'Train the Model',    desc: 'Configure parameters such as dataset profile, detector count, and Target FPR, then click Train. Watch the real-time log stream as the NSA builds its Self profile.' },
...
const FAQS = [
  ...
  { q: 'Why is my detection accuracy low?',
    a: 'Low accuracy can result from: (1) the model being trained on insufficient BENIGN data, (2) a mismatch between training and detection file schemas, or (3) calibration that is too conservative for the evaluation slice. Try retraining with more BENIGN data and adjust Target FPR during training if the project goal is higher recall and lower FNR.' },
];
...
const GLOSSARY = [
  ...
  { term: 'False Positive Rate (FPR)',     def: 'The proportion of normal flows incorrectly flagged as anomalies. AIS-Detect controls this through the Target FPR setting during training, then reuses the trained calibration for batch and live detection.' },
...
const ISSUES = [
  ...
  { problem: 'Detection returns 0 anomalies',
    solution: 'The model may be over-fitted to a very small dataset or trained with a calibration that is too conservative for the test slice. Try retraining with more BENIGN data and confirm the detection file uses the same dataset schema.' },
